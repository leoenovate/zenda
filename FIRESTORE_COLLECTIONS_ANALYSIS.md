...
```
User Input → Student Form → FirebaseService.addStudent()
  → Firestore: students/{docId}
  → Returns document ID
```

### 2. Attendance Recording Flow
```
Admin selects student & date → FirebaseService.recordAttendance()
  → Reads students/{studentId}
  → Updates attendanceHistory array
  → Writes back to students/{studentId}
```

### 3. Parent Login Flow
```
Parent enters phone → FirebaseService.getStudentsByParentPhone()
  → Query: students.where('fatherPhone' | 'motherPhone')
  → Returns matching students
```

### 4. Message Flow
```
User sends message → FirebaseService.sendMessage()
  → Creates: messages/{docId}
  → Real-time listener: messages.where('studentId')
  → Updates UI via Stream
```

### 5. Fingerprint Scanning Flow
```
Start scan → Listen: fingerprints.orderBy('timestamp').limit(1)
  → Scanner writes: fingerprints/{docId}
  → App detects new document
  → Copy fingerprint_sample to students/{studentId}.fingerprintData
```
...
service cloud.firestore {
  match /databases/{database}/documents {
    match /{document=**} {
      allow read, write: if true; // ⚠️ OPEN ACCESS - For development only
    }
  }
}
```
