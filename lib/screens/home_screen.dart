...
enum _HomeHeaderAction { reports, apiLogs }
...
class _HomeScreenState extends State<HomeScreen>
    ...
    setState(() {
      filteredStudents =
          students.where((student) {
            ...
            final matchesPeriod =
                selectedPeriod == null ||
                student.sessionIds.contains(selectedPeriod);

            // Filter by gender
            final matchesGender =
                selectedGender == null || student.gender == selectedGender;

            // Filter by attendance date
            bool matchesDate = true;
            if (selectedDate != null && student.attendanceHistory.isNotEmpty) {
              matchesDate = student.attendanceHistory.any(
                (attendance) =>
            ...
            if (selectedAttendanceStatus != null &&
                student.attendanceHistory.isNotEmpty) {
              ...
              matchesStatus = student.attendanceHistory.any(
                  ...
                  try {
                    ...
                    await FirebaseService.updateStudent(student.id!, {
                      ...
                      'sessionIds': List<String>.from(
                        (studentData['sessionIds'] as List?) ??
                            const <String>[],
                      ),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: Row(
                          ...
                          children:
                              attendanceData.take(7).map((attendance) {
                                return Column(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: switch (attendance.status) {
                                          AttendanceStatus.present => Colors
                                              .green
                                              .withOpacity(0.7),
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text(switch (attendance.status) {
                              ...
                              Row(
                                children: [
                                  ...
                                  PopupMenuButton<_HomeHeaderAction>(
                                    ...
                                    onSelected: (value) {
                                      switch (value) {
                                        case _HomeHeaderAction.reports:
                                          _openReports();
                                          break;
                                        case _HomeHeaderAction.apiLogs:
                                          _openApiLogs();
                                          break;
                                      }
                                    },
                                    itemBuilder:
                                        (context) => const [
                                          PopupMenuItem<_HomeHeaderAction>(
                                            value: _HomeHeaderAction.reports,
                                            child: Row(
                                              children: [
                                          ...
                                          PopupMenuItem<_HomeHeaderAction>(
                                            value: _HomeHeaderAction.apiLogs,
                                            child: Row(
                                              children: [
                                                Icon(
  ...
  Widget _buildStudentListItem(Student student) {
    final colorScheme = Theme.of(context).colorScheme;
    final periodAccent = colorScheme.onSurfaceVariant;
    return Stack(
      children: [
        ListTile(
          ...
          subtitle: Row(
            children: [
              Icon(Icons.event_note, color: periodAccent, size: 14),
              const SizedBox(width: 4),
              Text(
                student.sessionIds.isEmpty
                    ? 'No session'
                    : '${student.sessionIds.length} session${student.sessionIds.length == 1 ? '' : 's'}',
                style: TextStyle(color: periodAccent, fontSize: 12),
              ),
